---
title: "Jayachandran et al. (2017)"
description: "Jayachandran et al. (2017) as a concise running example for SpatialEffect2."
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Jayachandran et al. (2017)}
...
This example follows Jayachandran et al. (2017), a randomized intervention in
Uganda on forest conservation. Treatment is assigned at polygon units, and
outcomes come from spatial forest-loss raster data. We use this setting to
estimate how forest-loss effects vary with distance from each intervention
polygon.

<a class="btn btn-primary" href="../applications/jayachandran_etal_2017_bundle.zip" download>Download Jayachandran Data Bundle (.zip)</a>
...
```{r, fig.width=7.4, fig.height=5.6, fig.alt="Coarsened raster map of forest-loss outcomes in the Uganda setting."}
...
```{r, fig.width=7.4, fig.height=5.8, fig.alt="Forest-loss raster overlaid with control and treated intervention polygons."}
...
causal effect of switching one intervention node from control to treatment on
outcomes at distance `d`, marginalized over other nodes' assignments.

Here, distance is measured from each intervention polygon to surrounding raster
cells. A negative AME means lower forest loss around treated polygons relative
to control polygons at that distance.
...
dispatches to SpatialEffect2's S3 method, `plot.SpatialEffect()`.
Because this run sets `conley.se = 1`, the plot includes Conley confidence
intervals around the estimated AME curve.

```{r, fig.width=7.4, fig.height=5.2, fig.alt="Estimated AME curve by distance for the Jayachandran application using the SpatialEffect S3 plot method."}
graphics::plot(
  result,
  smooth = FALSE,
  ci.type = "conley",
...
In this run, AME estimates are negative at short distances and move toward zero
at larger distances. Substantively, this pattern is consistent with stronger
local treatment effects and weaker effects farther away.
...
N. E. Thomas (2017). "Cash for carbon: A randomized trial of payments for
ecosystem services to reduce deforestation." *Science* 357(6348): 267-273.
