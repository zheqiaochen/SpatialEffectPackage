...
#create raster
ras_Z_sf <- st_as_sf(as(map, "Spatial"))
...
grid_list <- circleMean(Zdata=Zdata,ras0 = ras, ras_Z = ras_Z_sf, dVec = dVec, nz = nrow(Zdata)) 
...
plot(ras, bty="n", box=FALSE, col = c("white", "darkgoldenrod4"), legend=FALSE, 
     ...
     cex.main=2,
     cex.axis = 2)
lines(ras_Z_sf[Zdata$treatment == 0, ], col="darkturquoise", lwd = 2)
lines(ras_Z_sf[Zdata$treatment == 1, ], col="goldenrod1", lwd = 2)

for (i in which(Zdata$treatment == 1)){
  buffer_treated <- sf::st_buffer(ras_Z_sf[i, ], dist=0.015 * 111000)
...
for (i in which(Zdata$treatment == 0)){
  buffer_control <- sf::st_buffer(ras_Z_sf[i, ], dist = 0.015 * 111000)
...
test2.result <- SpatialEffect2::SpatialEffectTest(result2.list, test.range = c(1, 5), smooth = 0, alpha = 0.1)
